...
# Tactic cache: (n, real_k, sm_count, n_alignment) -> dict[m_bucket -> tactic_tuple]
...
def _compute_tactic_for_m(m, n, real_k, sm_count, n_alignment):
    ...
    1. **swap_ab**: Swap A and B operands when M is small and N is
       output-dtype aligned, putting the larger N dimension on the M-axis to
       increase
       the number of CTAs.
...
def _select_sm100_mm_fp4_cute_dsl_tactic(m, n, real_k, sm_count, n_alignment=8):
    ...
    """
    cache_key = (n, real_k, sm_count, n_alignment)
    ...
    if tactic is None:
        tactic = _compute_tactic_for_m(
            m_bucket, n, real_k, sm_count, n_alignment
        )
